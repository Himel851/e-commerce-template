...
const ProductCard = ({ productDetails }) => {
  ...
  return (
    <div className="card bg-white shadow hover:shadow-lg border border-gray-200 relative">
      {/* Wishlist Icon */}
      ...
      {/* Product Image */}
      <Link href={`/product/${productDetails?.slug}`} className="relative block p-2">
        <div className="w-full aspect-square overflow-hidden rounded-lg image-zoom2">
          <Image
      ...
      {/* Product Info */}
      <Link href={`/product/${productDetails?.slug}`} className="block text-center mt-4">
        <p className="text-black font-normal text-sm xl:text-base line-clamp-1">
          {productDetails?.name}
        </p>
      ...
      <div className="px-6 py-4">
        {isOutOfStock ?
          <button className="btn bg-[#972222] w-full text-white">
            Out Of Stock
          </button> :
          <button className="btn bg-primary w-full text-white">
            Add to Cart
          </button>}
